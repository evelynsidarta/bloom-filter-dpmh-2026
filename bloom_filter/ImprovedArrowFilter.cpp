...
ImprovedBloomFilterMasks::ImprovedBloomFilterMasks() {
...
ImprovedBloomFilterMasks ImprovedArrowBloomFilter::masks;

#ifdef __AVX2__
// avx2 mask generation
inline __m256i ImprovedArrowBloomFilter::mask_avx2(__m256i hash) const {
    const __m256i full_mask = _mm256_set1_epi64x(static_cast<long long>(ImprovedBloomFilterMasks::fullMask));
    // mask_id = hash & (numberMasks - 1)
    const __m256i mask_id = _mm256_and_si256(hash, _mm256_set1_epi64x(ImprovedBloomFilterMasks::numberMasks - 1));
    // for the byte offset
    ...
    // rotation
    __m256i rotation = _mm256_srli_epi64(hash, ImprovedBloomFilterMasks::logNumberMasks);
...
inline __m256i ImprovedArrowBloomFilter::block_id_avx2(__m256i hash) const {
    // CHANGED: do one shift instead (see ImprovedArrowFilter.h for expanded explanation)
    return _mm256_srlv_epi64(hash, _mm256_set1_epi64x(static_cast<long long>(block_shift)));
}

// batch insert implementation
std::size_t ImprovedArrowBloomFilter::insertBatchImpl_avx2(
...
std::size_t ImprovedArrowBloomFilter::containsBatchImpl_avx2(
...
void ImprovedArrowBloomFilter::insertBatchImpl(const std::uint64_t* hash_array, std::size_t count) {
...
void ImprovedArrowBloomFilter::containsBatchImpl(const std::uint64_t* hash_array, std::uint8_t* result, std::size_t count) const {
    std::size_t processed = 0;
    // clear result buffer
    // CHANGED: no need to clear if not tail
    ...
    if (impl_mode == ImplMode::avx2) {
        ...
        result[processed >> 3] = 0;
    } else {
#endif
        // clear for tail
        std::fill(result, result + ((count + 7) >> 3), 0);
    }
    for (std::size_t i = processed; i < count; i++) {
        if (containsImpl(hash_array[i])) {
            result[i >> 3] |= static_cast<std::uint8_t>(1u << (i & 7u));
        }
    }
}